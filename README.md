...
它适合把一张自由白板变成可讲解、可投屏、可录制、可继续编辑的项目资产。产品主流程固定为“准备 → 录制 → 编辑 → 导出”。用户只需设定一个项目文件夹；白板、附件、原始录制、事件、逐字稿、字幕、编辑决策和最终成片都保存在该目录下。原始素材保持只读，剪辑和效果只写入项目时间线。
...
<p align="center">
  <img src="docs/assets/readme/21-v011-rich-sketch-project.png" alt="more-excalicord v0.1.1 实际载入论文工作流案例白板，项目卡片显示 A-rich-sketch、scene.excalidraw 已保存和 284 个元素" width="100%">
</p>

more-excalicord 的录制面板按“项目 → 提词器 / 讲稿 → 摄像头与麦克风 → 录制设置 → 录制效果 → 录制结果”排列。项目卡片集中显示项目名称、缩略路径和当前状态；“选择项目文件夹/更换项目”是唯一的项目目录选择动作。“显示位置”只在 Finder 中打开当前项目，“打开其他白板”只加载单个白板文件且不改变项目目录，“保存到项目”负责把实时元素、图片附件和 schema v2 清单写入同一个项目。四条链路保持独立，并以状态徽标区分待保存、已载入、已保存和操作失败。

录制前只呈现项目、讲解准备、摄像头与麦克风、录制设置和录制效果。摄像头与麦克风只决定输入设备、人像是否入片和显示位置；录制设置只决定范围、画幅、格式、背景和桌面图标；录制效果集中管理补光、人像优化、智能镜头和光标提示。逐字稿、字幕、非破坏剪辑等内容只在录后编辑器中出现；提词稿表示计划讲什么，逐字稿必须来自实际录音的 ASR 或明确导入的逐字稿文件。

录制开始后，主面板会收起并显示紧凑录制工具条。工具条可通过左侧手柄拖到屏幕内任意位置，并提供计时、提示词、暂停/继续和停止命令；“提示词”可在录制中打开或收起现有提词器。

<p align="center">
  <img src="docs/assets/readme/22-v011-recording-toolbar.png" alt="more-excalicord 正式录制后收起为悬浮录制条，仅显示时长、提示词、暂停和停止" width="100%">
</p>

每次新录制进入 `recordings/<session-id>/`，原片旁保存 `session.json` 与 `events.json`，事件统一使用录制开始后的毫秒时基。录制完成后可进入独立的录后编辑工作台：按实际录音生成或导入词级逐字稿，保留原始稿与校正版，生成字幕和剪辑建议，再配置镜头、光标、摄像头、画面和音频，最终输出 `exports/final.mp4`。浏览器直存依赖 `showDirectoryPicker`，桌面录制使用本地 Capture Agent；localStorage 只作为崩溃恢复缓存，并以项目根身份隔离，不能作为项目真值。

启动和切换项目时，以当前项目根目录中的 `project.excalicord.json` 为真值。空目录或无 manifest 的目录会进入新项目上下文，不会继承其他项目的录制、编辑、字幕或媒体引用；只有实际媒体文件存在，或当前会话仍持有有效 Blob 时，才显示“进入录后编辑”。
...
v0.1.1 的 AI 补光定位为 **Virtual Ring Light（屏幕环形补光灯）**：它不是 v0.2 才计划引入的人脸检测、分割或自动曝光 AI，而是在录制者主屏四周亮起可见的柔光灯圈，用屏幕真实发光改善人脸亮度。桌面/窗口原生录制时灯圈由 Capture Agent 的 native overlay 显示，并设置为不参与屏幕共享；浏览器兜底录制仍使用网页内补光层，因此选择整页/标签页录制时需要复查是否入镜。
...
| 隐藏桌面图标 | 录制整个屏幕时由 macOS Capture Agent 隐藏 Finder 桌面图标；停止、启动失败、页面退出或录制组件重启后恢复录制前状态 |
...
| 项目文件夹 | 项目卡片显示名称、缩略路径和状态，并提供选择/更换项目、显示位置、打开其他白板和保存到项目；录制、场景、附件、项目清单和字幕统一归档 |
| 摄像头合成 | 可把摄像头画中画合成进导出视频 |
| 摄像头位置 | 左上、右上、左下、右下 |
| AI 补光（屏幕补光灯） | v0.1.1 的 Virtual Ring Light：主屏四周显示跑道形圆角光带，只在边缘保留轻微柔化，亮度可调至全亮，中心白板、PPT 或代码保持不变；桌面/窗口原生录制由 Capture Agent 显示 native overlay，默认关闭 |
| 麦克风 | 可选择默认或指定麦克风，并显示实时音量 |
| 光标 | 可设置高亮形式、鼠标形状和点击提示音 |
| 智能镜头 | 幻灯片聚焦 + 鼠标智能聚焦 + 点击时聚焦 + 打字自动缩放；屏幕/窗口录制在录后编辑里生成和调整聚焦镜头 |
...
<p align="center">
  <img src="docs/assets/readme/23-v011-real-post-editor.png" alt="more-excalicord v0.1.1 使用实际 H.264 MP4 打开的录后编辑工作台，包含视频预览、剪辑检查器和多轨时间线" width="100%">
</p>
...
打开编辑器只会读取并在内存中校正媒体时长；未做任何编辑直接返回白板时，不会改写 `project.excalicord.json`。只有剪切、逐字稿校正、字幕、镜头或其他真实编辑使项目进入 dirty 状态后，编辑状态才会落盘。
...
### v0.1.1 实机复验记录（2026-08-24）

本轮使用仓库中的 `examples/more-paper-workflow-video-rich-sketch-20260815.excalidraw`，在 macOS 双显示器环境、真实摄像头/麦克风/屏幕录制权限下完成以下链路。媒体参数由 `ffprobe` 复核，最终成片同时由 QuickTime Player 实际打开。

| 路径 | 实际产物 | 结果 |
| --- | --- | --- |
| 指定 Chrome 窗口 + 摄像头合成 + 麦克风 | H.264/AAC，1436×1030，53.535 秒 | MP4 写入项目 `recordings/`，暂停/继续/停止有效 |
| 白板全景 + 麦克风 | H.264/AAC，1920×1080，44.106 秒 | 浏览器原片保存到独立 session 目录 |
| 扩展屏整屏 + 麦克风 | H.264/AAC，1920×1080，50.417 秒 | Native Capture Agent 录制并写入独立 session 目录 |
| 录后编辑 → 导出 | H.264/AAC，1920×1080，50.400 秒 | 生成 `exports/final.mp4`，QuickTime Player 成功打开 |
| 空项目 B 打开/关闭 | 0 个文件 | 不继承 A 的录制或编辑入口，不自动创建 manifest |
| 有效 MP4 项目打开/关闭编辑器 | manifest 哈希与 mtime 均不变 | 未修改不写盘 |

这组结果只代表上述设备、权限和案例文件的本次实测，不替代在目标机器上对摄像头位置、真实口播内容、系统音频、不同窗口来源和长时间录制稳定性的复验。

补光验收应单独看两个层面：录制者肉眼能看到屏幕边缘灯圈，并能感受到它确实在给人脸补光；同时检查导出的 MP4 中不应出现这圈灯。当前 v0.1.1 只承诺屏幕环形补光，不承诺人脸识别、人物居中、肤色保护或自动曝光，这些属于后续 AI 人像增强版本。

已知界面限制：在部分 macOS 双显示器布局中，点击“更换项目”后系统目录选择器可能已经创建在扩展屏坐标，但仍位于 Chrome 窗口后方。此时将 “Excalicord Capture” 前置即可继续选择；项目隔离仍以最终选中的目录和其中的 manifest 为准。
...
- 项目 A → 空项目 B → 项目 A 切换时，项目 ID、录制、编辑状态和媒体引用互不串用；
- 空项目只打开/关闭录制面板，不应创建 `project.excalicord.json`；
- 屏幕/窗口录制可用；
- 白板录制可用；
- 当前幻灯片聚焦录制可用；
- 摄像头画中画在屏幕上可见；
...
- 有效 MP4 项目打开编辑器后不做修改直接返回，manifest 的哈希和 mtime 应保持不变；
...
自动检查只能证明代码和部署链路没有明显断裂，不能替代真实白板、macOS 目录选择器、摄像头/麦克风/屏幕录制权限、真实浏览器录制、最终 MP4 下载和 QuickTime 回放。

...
more-excalicord 的录制面板按“项目 → 提词器/讲解准备 → 摄像头与麦克风 → 录制”排列。“设置项目文件夹…”建立新的项目上下文，保留当前白板和讲稿但不会带入上一项目的录制或编辑资产；“打开项目文件夹…”选择并加载完整项目；“打开 Excalidraw 文件…”只加载单个白板文件且不改变项目根；“在 Finder 中显示”和“保存白板”都是显性动作。保存白板会把实时元素、图片附件和 schema v2 清单写入同一个项目目录。
...
| 提词器 | 粘贴讲稿、从 SRT/VTT 导入讲稿、调速度、调字号、可选择录制时隐藏 | 需要稳定口播节奏的演示录制 |
...
录制面板把讲解录制需要的配置集中在一起。你可以先选画幅，再选录制范围，最后决定是否开启摄像头画中画、是否把摄像头合成进原始录制。项目文件夹状态位于“项目”区；画幅为 16:9 时输出尺寸为 1920×1080。停止后当前文件称为“原始录制”，可“保存录制”或“播放原始录制”；处理完成后在录后编辑工作台执行“导出成片”。
...
| 项目路径 | 用户可自由选择项目根目录，并显式打开白板、保存白板或在 Finder 中打开；录制、场景、附件、项目清单和字幕统一归档 |
| 摄像头合成 | 可把摄像头画中画合成进导出视频 |
| 摄像头位置 | 左上、右上、左下、右下 |
| 光标 | 录制中鼠标高亮 |
| 快捷键 | 开始、暂停、停止均有快捷键提示 |

录制功能的最终验收不以按钮状态为准，而以真实录制、保存原始录制并用本地播放器回放为准。尤其是画中画和当前幻灯片录制，应检查原始录制里是否真的保留人像、画面范围和切页效果。
...
页面出现 more-excalicord 悬浮栏后，至少检查一次：新增幻灯片、打开幻灯片总览、打开白板控制、打开录制面板。录制相关改动建议分别试录屏幕/窗口、白板画布和当前幻灯片三种范围；如果开启摄像头，还要确认画中画是否显示、是否按预期合成进原始录制。
...
| [v0.1.1 界面走查](docs/v0.1.1-ux-review.zh-CN.md) | 项目路径、录制、录后编辑和成片导出的使用逻辑与验收证据 |

...
more-excalicord 的录制面板按“项目 → 提词器 / 讲稿 → 摄像头画中画 → 录制”排列。项目卡片集中显示项目名称、缩略路径和当前状态；“选择项目文件夹/更换项目”是唯一的项目目录选择动作。“显示位置”只在 Finder 中打开当前项目，“打开其他白板”只加载单个白板文件且不改变项目目录，“保存到项目”负责把实时元素、图片附件和 schema v2 清单写入同一个项目。四条链路保持独立，并以状态徽标区分待保存、已载入、已保存和操作失败。
...
| 摄像头位置 | 左上、右上、左下、右下 |
| 屏幕柔光 | 摄像头画中画可选显示柔光圈，用屏幕给人脸补光；默认关闭 |
